...
fn render_cli_error(problem: &str) -> String {
    let mut lines = vec![i18n::t("error.title")];
    for (index, line) in problem.lines().enumerate() {
        let label = if index == 0 {
            i18n::t("error.problem")
        } else {
            i18n::t("error.problem_cont")
        };
        lines.push(format!("{label}{line}"));
    }
    lines.push(i18n::t("error.help"));
    lines.join("\n")
}

fn run() -> Result<(), Box<dyn std::error::Error>> {
    // Initialize locale from env before parsing args (--lang overrides in parse_args)
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    // Pre-scan for --lang to initialize locale early
    for i in 0..args.len() {
        if args[i] == "--lang" {
