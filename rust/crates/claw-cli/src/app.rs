...
impl SlashCommand {
    ...
    pub fn parse(input: &str) -> Option<Self> {
        ...
        Some(match command {
            ...
            "lang" => Self::Lang(parts.next().map(String::from)),
            other => Self::Unknown(other.to_string()),
        })
    }
}

fn slash_command_handlers() -> Vec<(SlashCommand, String)> {
    vec![
        (SlashCommand::Help, t("cmd.help.summary")),
        (SlashCommand::Status, t("cmd.status.summary")),
        (SlashCommand::Compact, t("cmd.compact.summary")),
        (SlashCommand::Lang(None), t("cmd.lang.summary")),
    ]
}
...
impl CliApp {
    ...
    pub fn run_repl(&mut self) -> io::Result<()> {
        ...
        println!("{}", t("app.interactive_mode"));
        println!("{}", t("app.help_hint"));
    ...
    fn dispatch_slash_command(
        ...
        match command {
            ...
            SlashCommand::Lang(lang_arg) => Self::handle_lang(lang_arg.as_deref(), out),
            SlashCommand::Unknown(name) => {
                writeln!(out, "{}", t_fmt("cmd.unknown", &[("name", &name)]))?;
                Ok(CommandResult::Continue)
            }
            _ => {
                writeln!(out, "{}", t("cmd.unavailable"))?;
                Ok(CommandResult::Continue)
            }
        }
    }
    ...
    fn handle_help(out: &mut impl Write) -> io::Result<CommandResult> {
        writeln!(out, "{}", t("cmd.available"))?;
        for (command, summary) in slash_command_handlers() {
            let name = match command {
                SlashCommand::Help => "/help",
                SlashCommand::Status => "/status",
                SlashCommand::Compact => "/compact",
                SlashCommand::Lang(_) => "/lang",
                _ => continue,
            };
    ...
    fn handle_lang(lang_arg: Option<&str>, out: &mut impl Write) -> io::Result<CommandResult> {
        match lang_arg {
            None => {
                let current = i18n::get_locale();
                let display = i18n::locale_display_name(&current);
                writeln!(out, "{}", t_fmt("cmd.lang.current", &[("lang", &format!("{current} ({display})"))]))?;
                writeln!(out, "{}", t("cmd.lang.usage"))?;
                writeln!(out)?;
                for code in i18n::SUPPORTED_LOCALES {
                    let display = i18n::locale_display_name(code);
                    let marker = if *code == current { " *" } else { "" };
                    writeln!(out, "  {code}  {display}{marker}")?;
                }
            }
            Some(lang) if i18n::SUPPORTED_LOCALES.contains(&lang) => {
                i18n::set_locale(lang);
                let display = i18n::locale_display_name(lang);
                writeln!(out, "{}", t_fmt("cmd.lang.switched", &[("lang", &format!("{lang} ({display})"))]))?;
            }
            Some(lang) => {
                writeln!(out, "{}", t_fmt("cmd.lang.invalid", &[("lang", lang)]))?;
            }
        }
        Ok(CommandResult::Continue)
    }
    ...
    fn handle_compact(&mut self, out: &mut impl Write) -> io::Result<CommandResult> {
        ...
        writeln!(
            out,
            "{}",
            t_fmt("cmd.compact.done", &[("count", &self.state.compacted_messages.to_string())])
        )?;
        Ok(CommandResult::Continue)
    }
    ...
    fn handle_stream_event(
        ...
        match event {
            StreamEvent::TextDelta(delta) => {
                if !*saw_text {
                    let _ =
                        stream_spinner.finish(&t("stream.response"), renderer.color_theme(), out);
                    *saw_text = true;
                }
            ...
            StreamEvent::ToolCallStart { name, input } => {
                ...
                let _ = tool_spinner.tick(
                    &t_fmt("stream.tool_running", &[("name", &name), ("input", &input)]),
            ...
            StreamEvent::ToolCallResult {
                ...
                let label = if is_error {
                    t_fmt("stream.tool_failed", &[("name", &name)])
                } else {
                    t_fmt("stream.tool_completed", &[("name", &name)])
                };
        ...
        match self.config.output_format {
            OutputFormat::Text => {
                writeln!(
                    ...
                    "\n{}",
                    t_fmt("usage.tokens", &[
                        ("input", &self.state.last_usage.input_tokens.to_string()),
                        ("output", &self.state.last_usage.output_tokens.to_string()),
                    ])
                )?;
            }
            OutputFormat::Json => {
    ...
    fn render_response(&mut self, input: &str, out: &mut impl Write) -> io::Result<()> {
        ...
        if saw_text {
            ...
            stream_spinner.finish(&t("stream.response"), self.renderer.color_theme(), out)?;
        }
