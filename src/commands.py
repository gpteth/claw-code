...
def execute_command(name: str, prompt: str = '') -> CommandExecution:
    ...
    if module is None:
        return CommandExecution(name=name, source_hint='', prompt=prompt, handled=False, message=t('msg.unknown_command', name=name))
...
def render_command_index(limit: int = 20, query: str | None = None) -> str:
    ...
    lines = [t('msg.command_entries', count=len(PORTED_COMMANDS)), '']
    if query:
        lines.append(t('msg.filtered_by', query=query))
        lines.append('')
