...
def execute_tool(name: str, payload: str = '') -> ToolExecution:
    ...
    if module is None:
        return ToolExecution(name=name, source_hint='', payload=payload, handled=False, message=t('msg.unknown_tool', name=name))
...
def render_tool_index(limit: int = 20, query: str | None = None) -> str:
    ...
    lines = [t('msg.tool_entries', count=len(PORTED_TOOLS)), '']
    if query:
